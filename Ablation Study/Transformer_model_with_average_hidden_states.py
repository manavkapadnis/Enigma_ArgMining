...
"""
    ...
    The input to the pretrained model is the combination of key_point, argument and topic.
...
def make_dataset(tokenizer, args, kps, topics, labels, max_len_input, model_with_no_token_types = model_with_no_token_types, model_name = 'roberta'):
    ...
    for arg, kp, topic, label in zip(args, kps, topics, labels) :
        ...
        arg = url.sub(r'',arg)
        kp = url.sub(r'',kp)
        topic = kp = url.sub(r'', topic)
        
        html=re.compile(r'<.*?>')
        arg = html.sub(r'',arg)
        kp = html.sub(r'',kp)
        topic = kp = html.sub(r'', topic)
        ...
        topic = kp = emoji_pattern.sub(r'', topic)
...
save_model, train_losses, val_losses = train_and_evaluate(train_dataset,df_train, model, save_model_folder, model_name = 'bert', batch_size = BATCH_SIZE, learning_rate = LEARNING_RATE, epochs = EPOCHS, accumulation_steps = ACCUMULATION_STEPS)
test_preds_path = predict_model (test_dataset,df_test, save_model,model_name = 'bert')
